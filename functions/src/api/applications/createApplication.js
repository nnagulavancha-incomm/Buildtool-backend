 ...
 * @Last Modified time: 2017-11-04 22:18:21
 */
...
module.exports = (req, res) => {
    ...
    ref.get().then((snapshot) => {
        // checking if app name is duplicating
        ...
        var setAppID = ref.doc(body.appName);
        var setData = setAppID.set(app).then((appRecord) => res.status(200).send(appRecord.data())).catch(error => res.status(500).send(err));
    }).catch(error => {
